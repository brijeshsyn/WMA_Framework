...
import com.wma.framework.report.TestRailAndExtentReporter;
import com.wma.framework.util.ExcelUtilities;
import com.wma.gurock.testtrail.APIClient;
import com.wma.gurock.testtrail.APIException;
...
public abstract class InitTest {
	...
	public void beforeSuite() {
	    ...
	    if (ConfigProvider.getInstance().getTestRail().equalsIgnoreCase("Yes")) {
	    	...
	    	client.setPassword(ConfigProvider.getInstance().getEmailPassword());
	    }
	}
...
public Map<String, String> getTestDataWhere(String testCaseTitle) {
 ...
 /**
  * To update the status of the test case
  * 
  * @param testCaseTitle
  * @param status
  */
 /*
  * public void updateTestStatus(String testCaseTitle, String status) { String
  * testDataFile = config().getTestDataFile().split(Pattern.quote("."))[0];
  * ExcelUtilities excel = new ExcelUtilities(config().getResultFolder() +
  * System.getProperty("file.separator") + testDataFile + "_" +
  * config().getTimeStamp() + ".xlsx"); excel.updateValue(config().getProduct(),
  * "Status", status, "TestCaseTitle='" + testCaseTitle + "'"); }
  */
